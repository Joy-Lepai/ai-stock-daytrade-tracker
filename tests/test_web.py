...
class WebTests(unittest.TestCase):
    ...
    def test_build_operator_runbook_payload_is_user_action_oriented(self):
        ...
        self.assertEqual(payload["operator_task_card"]["status_label"], "可盯盤：強烈買多 1 檔")
        self.assertEqual(payload["operator_task_card"]["first_step"], "先看強烈買多，再逐檔確認進場雷達與停損距離")
        self.assertEqual(payload["operator_task_card"]["do_not"], "不要追 high_risk")
        self.assertEqual(payload["operator_task_card"]["refresh"], "POST /refresh_watchlist")
    ...
    def test_operator_runbook_includes_front_category_no_signal_triage(self):
        ...
        self.assertIn("不是叫你主動放空", triage)

    def test_operator_runbook_explains_buy_without_strong_buy_as_wait_for_trigger(self):
        refresh_payload = {
            "generated_at": "2026-06-26T09:05:00+08:00",
            "market_mode": "intraday",
            "market_mode_label": "盤中",
            ...
            "operational_health": {
                ...
                "operator_decision": {
                    "decision": "等待",
                    "headline": "資料可用，但現在等觸發",
                    "first_action": "先看買多清單的下一步觸發條件",
                    "can_trade_now": False,
                    "can_use_intraday_signals": True,
                    "can_trust_strong_buy": False,
                },
                ...
                "do_now": ["先看買多清單的下一步觸發條件"],
                "do_not_do": ["不要提前追價"],
                "decision_checklist": ["是否站上 VWAP？"],
                "watch_readiness": "可看但需等觸發",
                "operator_steps": ["先看買多清單"],
                "refresh_plan": [],
                "next_action": {"label": "不需手動更新"},
                "can_use_dashboard": True,
                "can_show_strong_long": False,
        ...
        self.assertIn("買多 2 檔", triage)
        self.assertIn("不是完全沒有多方標的", triage)
        self.assertIn("尚未通過前不要提前進場", triage)
    ...
    def test_operator_page_renders_user_action_runbook(self):
        ...
        self.assertIn("開盤任務卡", html)
        self.assertIn("operator-task-status", html)
        self.assertIn("operator-task-first", html)
        self.assertIn("operator-task-do-not", html)
    ...
    def test_tw_advisor_page_has_stock_input_and_scan_api(self):
        ...
        self.assertIn("renderAdvisorPreOpenPlanCard", html)
        self.assertIn("開盤前單檔檢查卡", html)
        self.assertIn("現在不是即時買進判斷", html)
        self.assertIn("開盤後等什麼", html)
        self.assertIn("非盤中、資料非 live、缺 VWAP 或缺量比", html)
        self.assertIn("renderAdvisorTimeBucketCard", html)
        self.assertIn("單檔作戰時段", html)
        self.assertIn("開盤觀察 09:00-09:20", html)
        self.assertIn("不要追第一波急拉", html)
        self.assertIn("renderLimitUpPlaybookCard", html)
        self.assertIn("漲停 / 急拉作戰卡", html)
        self.assertIn("不要把接近漲停直接當成買多", html)
        ...
        self.assertIn("資料符合模式", html)
        self.assertIn("允許即時進場", html)
