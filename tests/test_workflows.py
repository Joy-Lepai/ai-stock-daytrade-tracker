...
class WorkflowTests(unittest.TestCase):
    def test_refresh_dashboard_workflow_prints_refresh_status_snapshot(self):
        ...
        self.assertIn("tee operational-health.txt", text)
        self.assertIn("Operational health check could not run.", text)
        self.assertIn("refresh-status.json", text)
        self.assertIn("operational-health.txt", text)
        self.assertIn("Upload refresh diagnostics", text)
        self.assertIn("uses: actions/upload-artifact@v4", text)
        self.assertIn("refresh-diagnostics", text)
