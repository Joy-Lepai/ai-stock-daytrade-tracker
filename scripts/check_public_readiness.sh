...
print(f"local_differs_from_origin: {payload.get('local_differs_from_origin')}")
print(f"ahead_count: {payload.get('ahead_count')}")
print(f"ahead_known: {payload.get('ahead_known')}")
print(f"public_runtime: {str(payload.get('public_runtime') or '-')[:12]}")
print(f"public_status: {payload.get('public_status') or '-'}")
print(f"public_reachable: {payload.get('public_reachable')}")
...
unpushed = payload.get("unpushed_commits") or []
if unpushed:
    print("unpushed_commits:")
    for item in unpushed[:10]:
        print(f"- {item}")
print("operator_gate:")
