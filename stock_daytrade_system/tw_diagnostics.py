    ...
    *,
    ...
    if missed > 0:
        return {
            ...
            "market_phase_summary": f"有 {total} 檔接近漲停 / 漲停，其中 {missed} 檔是真漏抓；優先檢查資料源、候選池與掃描門檻。",
    ...
    if data_missing >= max(2, total // 3):
        return {
            ...
            "market_phase_summary": f"有 {total} 檔接近漲停 / 漲停，但資料不足比例偏高；不可用來做即時進場判斷。",
    ...
    if total >= 10:
        if chase_risk >= max(3, total // 2):
            return {
                ...
                "market_phase_summary": f"今天有 {total} 檔接近漲停 / 漲停，且多數被列為追價高風險；盤面很熱，但不代表適合直接追。",
                "operator_priority": "先挑已看到但 high_risk 的股票，等拉回 VWAP 附近不破、停損距離縮小，再重新評估。",
            }
        return {
            "market_phase": "broad_limit_wave",
            "market_phase_label": "漲停潮",
            "market_phase_summary": f"今天有 {total} 檔接近漲停 / 漲停，盤面動能明顯；重點是分辨真強續攻與追價陷阱。",
